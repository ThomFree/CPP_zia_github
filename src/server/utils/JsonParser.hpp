...
public:
	...
	void makeJsonFromConfig(const dems::config::Config &conf, const std::string &fileName);
...
private:
	...
	void writeFromConfig(const std::pair<const std::string, dems::config::ConfigValue> &obj, TYPE type, json &file, bool isArray);
	void traverseObj(const dems::config::ConfigObject &conf, json &jsonFile, bool isArray);
	void traverseArray(const dems::config::ConfigArray &array, json &jsonFile);
